...
if(isset($_GET["tkn"]) && $_GET["tkn"]==$_SESSION["tkn"]){
	...
	$update=mysqli_query($con, "update file set is_deleted=1 where id=$id");
	if ($update) {
		// echo "Eliminado exitosamente!";
		header("location: ../myfiles?movesuccess");
	} else {
		// echo "Hubo un error al eliminar ";
		header("location: ../myfiles?moveerror");
	}

}else{

	// echo "Permiso Denegado!";
	header("location: ../myfiles?moveinvalid");
}
