    ...
    <section class="content">
        ...
        <div class="row">
            ...
            <div id="tabla_original" class="col-md-12">
                ...
                //CONTROL DE USUARIO ADMINSTRADOR Y PUBLICO
                if ($is_public == 0) {
                    if ($folder == null) {
                    ...
                    if ($folder == null) {
                        if (isset($_GET["q"]) && $_GET["q"] != "") {
                            ...
                            $files = mysqli_query($con, "select * from file where folder_id is NULL and is_public=1 and (filename like '%$q%' or description like '%$q%') and is_deleted=0 order by is_folder desc, filename asc");
                        } else {

                            $files = mysqli_query($con, "select * from file where is_deleted=0 and folder_id is NULL and is_public=1 order by is_folder desc, filename asc");
                        }
                    } else {
                        // search
                        if (isset($_GET["q"]) && $_GET["q"] != "") {
                            ...
                            foreach ($folder as $folder_key) {
                                $folder_key_id = $folder_key['id'];
                                $files = mysqli_query($con, "select * from files where folder_id=$folder_key_id and is_public=1 and is_deleted=0 and  (filename like '%$q%' or description like '%$q%') order by created_at desc");
                            }
                        } else {
                            // folder/folder/file.php
                            foreach ($folder as $key_folder) {
                                $key_folder_id = $key_folder['id'];
                                // $files=mysqli_query($con,"select * from file where folder_id=$key_folder_id and is_deleted=0 order by created_at desc");
                                $files = mysqli_query($con, "select * from file where folder_id=$key_folder_id and is_public=1 and is_deleted=0 and status=1 order by created_at desc");
                            }
