...
if ($folder != null) {
    foreach ($folder as $claves) {
        ...
        $f = $claves['id'];
        $u_id = $claves['user_id'];
        $perm = mysqli_query($con, "select * from permision where user_id=$u and file_id=$f");
        $go = false;
            ...
            <?php if ($folder != null) :
                function getparent($folder)
                {
                    ...
                    foreach ($folder as $key) {
                        ...
                        echo "<li><a href='myfiles?folder=" . $function_code . "'>" . $function_filename . "</a></li>";
                    }
                }
                getparent($folder);
            endif;
            ...
            <div id="tabla_original" class="col-md-12">
                ...
                if ($folder == null) {
                    if (isset($_GET["q"]) && $_GET["q"] != "") {
                        ...
                        $files = mysqli_query($con, "select * from file where user_id=$S_user_id and folder_id is NULL and (filename like '%$q%' or description like '%$q%') and is_deleted=0 order by is_folder desc, filename asc");
                    } else {
                        $S_user_id = $_SESSION["user_id"];
                        $files = mysqli_query($con, "select * from file where user_id=$S_user_id and is_deleted=0 and folder_id is NULL order by is_folder desc, filename asc");
                    }
                } else {
                    // search
                    if (isset($_GET["q"]) && $_GET["q"] != "") {
